...
def objective(
    ...
    ppl_predictor = get_ppl_predictor_surrogate(
        search_space, base_path="../../HW-GPT-Bench/"
    )
    perplexity = ppl_predictor(
        arch_feature_map_ppl_predictor.to(device_run).unsqueeze(0)
    )
    hw_predictor = get_hw_predictor_surrogate(
        search_space, device, objective, base_path="../../HW-GPT-Bench/"
    )
    ...
    report(perplexity=ppl, hw_metric=hw_metric)

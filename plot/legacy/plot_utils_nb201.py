...
def get_min_max_metrics_from_observed_data(data_1):
    ...
    for experiment, data_per_metric_device_pair in data_1:
        ...
        for baseline_key, dataframe_per_baseline in data_per_metric_device_pair.items():
            ...
            mean_df = pd.concat(trimmed_dfs).groupby(level=0).mean()
            data_per_metric_device_pair[baseline_key] = mean_df

            current_max_perplexity = mean_df["error"].max()
            current_min_perplexity = mean_df["error"].min()

            current_max_hw_metric = mean_df["latency"].max()
            # print(baseline_key)
            # if baseline_key == "gpt-4o-mini_energies_h100":
            #    print(mean_df[:100])
            # print(f"current_max {current_max_perplexity}")
            # print(f"current_max {current_max_perplexity}")
            # print(f"current_max {current_max_perplexity}")
            current_min_hw_metric = mean_df["latency"].min()
...
def normalize_data(fvals, min_max_metrics):
    ...
    fvals["latency"] = (fvals["latency"] - min_hw_metric) / (
        max_hw_metric - min_hw_metric
    )
    fvals["error"] = (fvals["error"] - min_perplexity) / (

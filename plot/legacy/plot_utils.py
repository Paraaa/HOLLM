...
def extract_metadata_from_result_file(filepath):
    ...
    first_word = parts[0]  # LSBO - baseline
    second_word = parts[1]  # energies - metric
    seed = parts[-1].split(".")[0]  # 31415927 - seed
    device = "_".join(parts[2:-1])  # cpu_amd_7452 - device
...
def get_min_max_metrics_from_observed_data(data_1):
    ...
    for experiment, data_per_metric_device_pair in data_1:
        min_perplexity = 1000000000
        max_perplexity = 0
        min_hw_metric = 1000000000
        max_hw_metric = 0
        ...
        for baseline_key, dataframe_per_baseline in data_per_metric_device_pair.items():
            ...
            current_max_perplexity = all_dfs["perplexity"].max()
            current_min_perplexity = all_dfs["perplexity"].min()

            current_max_hw_metric = all_dfs["hw_metric"].max()
            current_min_hw_metric = all_dfs["hw_metric"].min()
...
def normalize_data(fvals, min_max_metrics):
    ...
    fvals["hw_metric"] = (fvals["hw_metric"] - min_hw_metric) / (
        max_hw_metric - min_hw_metric
    )
    fvals["perplexity"] = (fvals["perplexity"] - min_perplexity) / (
        max_perplexity - min_perplexity
    )

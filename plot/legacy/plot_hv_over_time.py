...
from plot.extract_statistics_from_surrogate import (
...
from plot.legacy.plot_settings import get_visuals
...
def create_hv_over_time_plot(model_name, data, img_path):
    ...
    ax.set_title("Hypervolume over Time for mooLLM on HWGPT (H100)", fontsize=18)
    ...
    ax.set_xticks([5, 6, 7, 20, 40, 60, 80, 100])
    fig.tight_layout()
...
def main():
    ...
    for experiment, data_per_metric_device_pair in data:
        directory_path = (
            f"./ablation_plots_hwgpt/hypervolume_over_time/{save_directory}"
        )

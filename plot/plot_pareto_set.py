...
from plot_settings import COLORS, LINE_STYLES, MARKERS
...
def create_pareto_set_plot(
    ...
    for seed, methods in data.items():
        ...
        for i, method_name in enumerate(method_names):
            ...
            # costs = np.array(method_df).reshape(
            #     1, method_df.shape[0], method_df.shape[1]
            # )

            # surfs = get_empirical_attainment_surface(costs=costs, levels=levels)
            # eaf.plot_surface_with_band(ax, color=color, label=method_name, surfs=surfs)
            ...
            if len(non_pareto_df) > 0:
                ax.scatter(
                    ...
                    s=30,
                    alpha=0.3,
                    label=f"{method_name} (non-Pareto)",
                )

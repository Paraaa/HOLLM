#!/bin/bash

# python ./plot/plot_hv_over_time.py \
#     --benchmark NB201 \
#     --title "Hypervolume (NB201 Titanx)" \
#     --filter "titanx" \
#     --columns "error,latency" \
#     --trials 60 \
#     --data_path ./results/NB201/ \
#     --filename "Titanx" \
#     --normalization_method "nb201"\
#     --nb201_device_metric "titanx_256_latency"\
#     --blacklist "QwQ-32B,Legacy-Prompt,Qwen2.5-32B,NSGA2"


# python ./plot/plot_hv_over_time.py \
#     --benchmark NB201 \
#     --title "Hypervolume (NB201 Titanx)" \
#     --filter "titanx" \
#     --columns "error,latency" \
#     --trials 80 \
#     --data_path ./results/NB201/ \
#     --filename "TitanxQwen" \
#     --normalization_method "nb201"\
#     --nb201_device_metric "titanx_256_latency"\
#     --blacklist "QwQ-32B,Legacy-Prompt,GPT-4o Mini"

# python ./plot/plot_hv_over_time.py \
#     --benchmark NB201 \
#     --title "Hypervolume (NB201 FPGA)" \
#     --filter "fpga" \
#     --columns "error,latency" \
#     --trials 100 \
#     --data_path ./results/NB201/ \
#     --filename "fpga"


# python ./plot/plot_hv_over_time.py \
#     --benchmark ZDT \
#     --title "Hypervolume (ZDT3)" \
#     --filter "zdt3" \
#     --columns "f1,f2" \
#     --trials 100 \
#     --data_path ./results/ZDT/ \
#     --filename "ZDT3"\
#     --blacklist "QwQ-32B,Legacy-Prompt,GPT-4o-mini,LSBO,RS,NSGA2,RSBO"


# python ./plot/plot_hv_over_time.py \
#     --benchmark WELDED_BEAM \
#     --title "Hypervolume (Welded Beam)" \
#     --filter "welded_beam" \
#     --columns "F1,F2" \
#     --trials 45 \
#     --data_path ./results/WELDED_BEAM/ \
#     --filename "WELDED_BEAM"\
#     --blacklist ""\
#     --y_lim "0.99" "1"\

# python ./plot/plot_hv_over_time.py \
#     --benchmark HWGPT \
#     --title "Hypervolume (HWGPT H100)" \
#     --filter "h100" \
#     --columns "Latency,Perplexity" \
#     --trials 50 \
#     --data_path ./results/HWGPT/ \
#     --filename "H100"



# python ./plot/plot_hv_over_time.py \
#     --benchmark ZDT \
#     --title "Hypervolume (ZDT1)" \
#     --filter "zdt1" \
#     --columns "F1,F2" \
#     --trials 50 \
#     --data_path ./results/ZDT1/ \
#     --filename "ZDT1"

# python ./plot/plot_hv_over_time.py \
#     --benchmark ZDT \
#     --title "Hypervolume (ZDT2)" \
#     --filter "zdt2" \
#     --columns "F1,F2" \
#     --trials 50 \
#     --data_path ./results/ZDT2/ \
#     --filename "ZDT2"

# python ./plot/plot_hv_over_time.py \
#     --benchmark ZDT \
#     --title "Hypervolume (ZDT3)" \
#     --filter "zdt3" \
#     --columns "F1,F2" \
#     --trials 60 \
#     --data_path ./results/ZDT3/ \
#     --filename "ZDT3"\
#     --blacklist "QwQ-32B,Legacy-Prompt,mooLLM-Int-ToT (Gemini 1.5 Flash),RS,NSGA2,RSBO,LSBO,GPT-4o Mini"


# python ./plot/plot_hv_over_time.py \
#     --benchmark ZDT \
#     --title "Hypervolume (ZDT4)" \
#     --filter "zdt4" \
#     --columns "F1,F2" \
#     --trials 50 \
#     --data_path ./results/ZDT4/ \
#     --filename "ZDT4"

# python ./plot/plot_hv_over_time.py \
#     --benchmark ZDT \
#     --title "Hypervolume (ZDT6)" \
#     --filter "zdt6" \
#     --columns "F1,F2" \
#     --trials 50 \
#     --data_path ./results/ZDT6/ \
#     --filename "ZDT6"